#!/usr/bin/env python3
"""
Interfaz gráfica para el generador de G-code con trazos a mano alzada
Permite seleccionar imágenes desde el explorador de archivos y configurar parámetros
"""
...
class GCodeGeneratorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador G-code Trazos a Mano Alzada")
        ...
        self.z_base = tk.DoubleVar(value=0.2)
        self.z_variation = tk.DoubleVar(value=0.8)
    ...
    def setup_ui(self):
        ...
        # Título
        title_label = ttk.Label(main_frame, text="Generador G-code Trazos a Mano Alzada", 
                               font=('Arial', 14, 'bold'))
        ...
        ttk.Entry(files_frame, textvariable=self.input_file, state='readonly').grid(
            row=0, column=1, sticky=(tk.W, tk.E), padx=(5, 5), pady=(0, 5))
        ttk.Button(files_frame, text="Buscar...", command=self.browse_input_file).grid(
            row=0, column=2, pady=(0, 5))
        
        # Archivo de salida
        ttk.Label(files_frame, text="Archivo G-code:").grid(row=1, column=0, sticky=tk.W)
        ttk.Entry(files_frame, textvariable=self.output_file).grid(
            row=1, column=1, sticky=(tk.W, tk.E), padx=(5, 5))
        ttk.Button(files_frame, text="Guardar como...", command=self.browse_output_file).grid(
            row=1, column=2)
        
        # Sección de parámetros del canvas
        ...
        ttk.Spinbox(canvas_frame, from_=10, to=1000, textvariable=self.canvas_width, 
                   width=10).grid(row=0, column=1, padx=(5, 20))
        
        ttk.Label(canvas_frame, text="Alto:").grid(row=0, column=2, sticky=tk.W)
        ttk.Spinbox(canvas_frame, from_=10, to=1000, textvariable=self.canvas_height, 
                   width=10).grid(row=0, column=3, padx=(5, 0))
        
        # Sección de parámetros Z
        ...
        ttk.Spinbox(z_frame, from_=0.1, to=50, increment=0.1, textvariable=self.z_safe, 
                   width=10, format="%.1f").grid(row=0, column=1, padx=(5, 20))
        
        ttk.Label(z_frame, text="Altura base:").grid(row=0, column=2, sticky=tk.W)
        ttk.Spinbox(z_frame, from_=0.0, to=10, increment=0.1, textvariable=self.z_base, 
                   width=10, format="%.1f").grid(row=0, column=3, padx=(5, 0))
        
        ttk.Label(z_frame, text="Variación Z:").grid(row=1, column=0, sticky=tk.W, pady=(5, 0))
        ttk.Spinbox(z_frame, from_=0.1, to=5, increment=0.1, textvariable=self.z_variation, 
                   width=10, format="%.1f").grid(row=1, column=1, padx=(5, 0), pady=(5, 0))
        ...
        ttk.Spinbox(speed_frame, from_=100, to=5000, textvariable=self.feed_rate, 
                   width=10).grid(row=0, column=1, padx=(5, 20))
        
        ttk.Label(speed_frame, text="Desplazamiento:").grid(row=0, column=2, sticky=tk.W)
        ttk.Spinbox(speed_frame, from_=500, to=10000, textvariable=self.travel_speed, 
                   width=10).grid(row=0, column=3, padx=(5, 0))
        ...
        ttk.Button(buttons_frame, text="Generar G-code", command=self.generate_gcode, 
        ...
        # Mensaje inicial
        self.log_message("¡Bienvenido al Generador de G-code!\nSelecciona una imagen para comenzar.")
    ...
    def browse_input_file(self):
        ...
        if filename:
            ...
            if not self.output_file.get():
                ...
                output_name = f"{base_name}_handdrawn.gcode"
    ...
    def generate_gcode_thread(self):
        """Ejecutar la generación de G-code en un hilo separado"""
        try:
            # Crear el generador con los parámetros de la GUI
            generator = HandDrawnGCodeGenerator(
                ...
                z_draw_base=self.z_base.get(),
                z_variation=self.z_variation.get(),
            ...
            messagebox.showinfo("Éxito", 
                              f"G-code generado exitosamente!\n\nArchivo: {self.output_file.get()}")
...
def main():
    ...
    style = ttk.Style()
    try:
        style.theme_use('winnative')  # Para Windows
    except:
